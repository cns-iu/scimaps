...
function getNewsItems(numberOfNewsItems: number, date: string = '2021-06-03') {
...
describe('NewsResolverService', () => {
  ...
  it('should give correct relative image path', () => {
    ...
    expect(newImagePath).toEqual(`assets/${service.directory}/2021/06-03/${slug}/${newsItem.thumbnail}`);
  });
  ...
  it('should give correct relative pdf path', () => {
    ...
    const slug = toSlug(newsItem.title);
    expect(newImagePath).toEqual(`assets/${service.directory}/2021/06-03/${slug}/${newsItem.pdfLink}`);
  });
